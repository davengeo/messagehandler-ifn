...
.PHONY: py-docker
py-docker:  py-activate ## Create a docker image with the tag name of the app:version
	rm -rf .env ; touch .env
	echo DIST_NAME=$(call get_setup_value,name) >> .env
	echo DIST_VERSION=$(call get_setup_value,version) >> .env
	docker-compose up
